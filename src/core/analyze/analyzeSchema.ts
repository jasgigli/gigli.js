      ...
      if (node.rules) {
        ...
        for (const rule of node.rules) {
          if (
            rule.name === "min" &&
            rule.params &&
            rule.params.value !== undefined
          )
            min = Number(rule.params.value);
          if (
            rule.name === "max" &&
            rule.params &&
            rule.params.value !== undefined
          )
            max = Number(rule.params.value);
        }
        if (min !== undefined && max !== undefined && min > max) {
          issues.push(
            `Impossible rule at ${path.join(".") || "root"}: min (${min}) > max (${max})`,
          );
        }
      }
    }
      ...
      for (const step of node.steps) {
        if (step.type === "dispatch") {
          ...
          if (!step.cases || Object.keys(step.cases).length === 0) {
            issues.push(
              `Unreachable dispatch at ${path.join(".") || "root"}: no cases defined`,
            );
          }
        }
      ...
      if (node.refinements) {
        for (const refine of node.refinements) {
          if (typeof refine.fn !== "function") {
            issues.push(
              `Invalid refinement at ${path.join(".") || "root"}: not a function`,
            );
          }
