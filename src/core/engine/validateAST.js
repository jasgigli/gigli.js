    ...
    return __awaiter(this, arguments, void 0, function* (node, value, context = {}) {
        ...
        if (node.type === 'object') {
            ...
            for (const key in node.fields) {
                ...
                const res = yield validateAST(fieldNode, fieldValue, context);
                out[key] = res.value;
        ...
        if (node.type === 'array') {
            ...
            for (let i = 0; i < value.length; i++) {
                const res = yield validateAST(node.element, value[i], context);
                out[i] = res.value;
                trace.push(...res.trace);
                if (!res.valid) {
                    valid = false;
        ...
        if (node.type === 'pipeline') {
            ...
            for (const step of node.steps) {
                if (step.type === 'transform') {
                    ...
                    if (schema) {
                        const res = yield validateAST(schema, pipelineValue, context);
                        pipelineTrace.push(...res.trace);
                        pipelineValue = res.value;
                        if (!res.valid) {
                            pipelineValid = false;
